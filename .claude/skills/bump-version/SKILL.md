...
4. **Commit the changes**: Run `git add . && git commit -m "chore: bump version to v<new-version>"`.
5. **Create the git tag**: Run `git tag v<new-version>`.
6. **Push to remote**: Instruct the user to push the changes and the tag to both `origin` and `github` remotes using:
   ...
   git push github master && git push github v<new-version>
   ```